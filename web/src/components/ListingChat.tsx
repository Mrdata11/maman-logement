...
export function ListingChat({ listing, evaluation }: ListingChatProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="w-full p-4 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-950/30 dark:to-purple-950/30 border border-violet-200 dark:border-violet-800 rounded-lg text-left hover:shadow-md transition-shadow"
      >
  ...
  return (
    <div className="border border-violet-200 dark:border-violet-800 rounded-lg overflow-hidden">
      ...
      {/* Messages */}
      <div className="max-h-80 overflow-y-auto p-4 space-y-3 bg-white dark:bg-slate-800">
        {messages.length === 0 && (
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTED_QUESTIONS.map((q, i) => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 rounded-full border border-violet-200 dark:border-violet-700 hover:bg-violet-100 dark:hover:bg-violet-900/50 transition-colors"
                >
                  {q}
                </button>
              ))}
      ...
      {/* Input */}
      <div className="flex gap-2 p-3 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
        <input
          type="text"
          value={input}
          ...
          className="flex-1 px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md text-sm bg-white dark:bg-slate-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-400"
          disabled={isLoading}
        />
        <button
          onClick={() => handleSend()}
          disabled={!input.trim() || isLoading}
          className="px-4 py-2 bg-violet-600 text-white rounded-md text-sm hover:bg-violet-700 disabled:opacity-50 transition-colors"
        >
          Envoyer
        </button>
      </div>
